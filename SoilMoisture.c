 ...
 */
#define IADC_INPUT_0_PORT_PIN     iadcPosInputPortCPin7;
...
#define SENSOR_POWER_2_PORT      gpioPortD
#define SENSOR_POWER_2_PIN       3
...
void initGPIO(void)
{
  ...
  // Show sample completion state on LED0
  GPIO_PinModeSet(SENSOR_POWER_2_PORT, SENSOR_POWER_2_PIN, gpioModePushPull, 0);
...
void initPRS(void)
{
 ...
 // PRS_PinOutput(PRS_CHANNEL , prsTypeAsync,SENSOR_POWER_2_PORT , SENSOR_POWER_2_PIN);
}
...
void initIADC(void)
{
  ...
  init.warmup =  iadcWarmupKeepInStandby;//iadcWarmupNormal;
...
void initLETIMER(void)
{
  ...
  uint32_t topValue = (CMU_ClockFreqGet(cmuClock_LETIMER0) / LETIMER_FREQ)/LETIMER_FREQ_DIV;
